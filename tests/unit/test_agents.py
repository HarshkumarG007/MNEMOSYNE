...
async def test_message_bus_pubsub() -> None:
    ...
    await bus.publish("test_topic", AgentMessage(id="1", sender="test", topic="test_topic", payload={"value": "A"}))
...
async def test_message_bus_dlq() -> None:
    ...
    await bus.publish("fail_topic", AgentMessage(id="2", sender="test", topic="fail_topic", payload={}))
