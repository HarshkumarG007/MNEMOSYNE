        ...
        for file_path in files:
            try:
                ...
                artifact = {
                    ...
                    "mime_type": mime_type,
                    "status": "extracted",
                    "content_hash": "dummy_hash_" + str(uuid.uuid4())[:8]
                }
                artifacts.append(artifact)
                
                # Publish progress for this specific file
                await bus.publish("progress", AgentMessage(
                    id=str(uuid.uuid4()),
                    sender=self.name,
                    topic="progress",
                    payload={"file": file_path, "status": "success"}
                ))
                
            except Exception as e:
                logger.error(f"[IngestionAgent] Error processing {file_path}: {e}")
                # Error isolation: one file fails, rest continue
                await bus.publish("error", AgentMessage(
                    id=str(uuid.uuid4()),
                    sender=self.name,
                    topic="error",
